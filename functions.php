...
//    echo 'currencycode: '.$ocart['currencycode'].'<br>';
...
function ocart_currency_label($code) {
	switch ($code) {
		case 'CLP':
			return sprintf(__('Peso Chileno (%s)','ocart'), $code);
			break;
		case 'USD':
		...
		<ul class="navi">
			...
			if  ($taxonomies) {
				foreach ($taxonomies as $taxonomy ) {
					if (!in_array($taxonomy, array( 'brand', 'color', 'size', 'collection'))) {
...
function ocart_product_avg_rating() {
	...
	if ($post_comments) {
		...
		echo '<div class="clear"></div><div class="product-rating-note">'.sprintf(__('Avg. customer rating: <strong>%s</strong> / 5','ocart'), formatocurrency((double)($avg_total / count($post_comments)),0,',','.'), count($post_comments)).'</div>';
...
function ocart_get_currency_symbol() {
	...
	switch($currency_code) {
		case 'CLP': $code = 'CLP $'; break;
		case 'USD': $code = 'US $'; break;
		case 'EUR': $code = '€'; break;
		case 'GBP': $code = '£'; break;
...
function ocart_print_saved_coupons() {
	...
	foreach($_SESSION as $sess => $array) {
		if (strstr($sess, 'coupon_')) {
			if ($array['type'] == 2) {
				$array['amount'] = formatocurrency(ocart_real_coupon_value($array['amount']),0,',','.');
			}
			if ($array['type'] == 1) {
				$array['amount'] = formatocurrency($array['amount'],0,',','.');
			}
...
function ocart_show_coupons_in_cart() {
	...
	foreach($_SESSION as $sess => $array) {
		if (strstr($sess, 'coupon_')) {
			if ($array['type'] == 2) {
				$array['amount'] = formatocurrency(ocart_real_coupon_value($array['amount']), 2);
			}
			if ($array['type'] == 1) {
				$array['amount'] = formatocurrency($array['amount'], 2);
			}
...
function ocart_show_price($price, $xchange=true) {
	if (isset($price) && $price != '') {
		...
		return formatocurrency($price,0,',','.');
	} else {
		return formatocurrency(0);
	}
}

/************************************************************
Mostrar Numeros de una manera que funcione el filtro
************************************************************/
...
function ocart_payable($post_id, $force_currency = false) {
	...
	if ( $force_currency == true ) {
		...
		$payable = formatocurrency((float)$payable, 2, ',', '.');
...
function formatocurrency($numero=0,$decimales=0,$sepdec=0,$septhousand=0){
