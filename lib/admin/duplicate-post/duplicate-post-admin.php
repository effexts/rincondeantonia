...
function duplicate_post_plugin_upgrade() {
	...
	if (empty($installed_version)) { // first install
		...
		add_option('duplicate_post_copyattachments','1');
...
function duplicate_post_make_duplicate_link_row($actions, $post) {
	if (duplicate_post_is_current_user_allowed_to_copy() && $post->post_type == 'product') {
		$actions['clone'] = '<a href="'.duplicate_post_get_clone_post_link( $post->ID , 'display', false).'" title="'
		. esc_attr(__("Duplicate this product", 'ocart'))
		. '">' .  __('Duplicate', 'ocart') . '</a>';
	}
	return $actions;
}
...
<div id="duplicate-action">
	<a class="submitduplicate duplication"
		href="<?php echo duplicate_post_get_clone_post_link( $_GET['post'] ) ?>"><?php _e('Copy to a new product', 'ocart'); ?>
	</a>
</div>
		<?php
	}
...
function duplicate_post_save_as_new_post($status = ''){
	if (! ( isset( $_GET['post']) || isset( $_POST['post'])  || ( isset($_REQUEST['action']) && 'duplicate_post_save_as_new_post' == $_REQUEST['action'] ) ) ) {
		wp_die(__('No post to duplicate has been supplied!', 'ocart'));
	}
	...
	if (isset($post) && $post!=null) {
		...
		wp_die(esc_attr(__('Copy creation failed, could not find original:', 'ocart')) . ' ' . $id);
	}
}
...
function duplicate_post_add_plugin_links($links, $file) {
	if ( $file == plugin_basename(dirname(__FILE__).'/duplicate-post.php') ) {
		$links[] = '<a href="http://lopo.it/duplicate-post-plugin">' . __('Donate', 'ocart') . '</a>';
		$links[] = '<a href="http://lopo.it/duplicate-post-plugin">' . __('Translate', 'ocart') . '</a>';
	}
	return $links;
}
?>